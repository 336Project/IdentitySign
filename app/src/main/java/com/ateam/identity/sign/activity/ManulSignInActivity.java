    ...
    private void signin() {
        HRequestCallback<HBaseObject> request = new HRequestCallback<HBaseObject>() {
            ...
            public HBaseObject parseJson(String jsonStr) {
                ...
                return (HBaseObject) JSONParse.jsonToObject(jsonStr, type);
            }
            ...
            public void onSuccess(HBaseObject result) {
                ...
                if (!result.isSuccess()) {
                    for (int i = 0; i < mCommitStudent.size(); i++) {
                        ...
                        signRecord.setCardNum(mCommitStudent.get(i).getCardNum());
                        signRecord.setAttendanceDate(SysUtil.time2Long(mTvTime.getText().toString()));
